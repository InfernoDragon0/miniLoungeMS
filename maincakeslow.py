...
def runCV():
    ...
    while True:
        ...
        #get numpy where for [238 238 255]
        # mask = numpy.nonzero(datasmol[0] == [238, 238, 255])
        # print(length)
        # cv2.rectangle(datashow, (cakestartY, cakestart), (cakeendY, cakeend), (0, 0, 255), 2)

        # GET LENGTH
        if (length == -1):
            #set the length if its empty
            print("getting length")
            cakedata = numpy.where((datasmol[:,:,0] == 238) & (datasmol[:,:,1] == 238) & (datasmol[:,:,2] == 255))
            length = len(cakedata[1]) - offset
            ...
            #example if currentedge is 260, then previous edge is between 257 and 263 (260 +- 3)
            offset2 = 1
            if (currentEdges[0] >= previousEdges[0] - offset2 and currentEdges[0] <= previousEdges[0] + offset2):
                #press space
